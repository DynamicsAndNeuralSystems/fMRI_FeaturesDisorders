#------------------------------------
# This script runs catch22 for one region across 
...
parser$add_argument("--brain_region", help="Name of brain region to run analysis for.")
...
brain_region <- "ctx-lh-entorhinal"
noise_proc <- "AROMA+2P"
noise_label <- gsub("\\+", "_", noise_proc)
...
# Filter to the user-specified brain region
TS_data_region <- subset(TS_data, Brain_Region==brain_region)

# Run catch22 using theft
feature_matrix <- calculate_features(data = TS_data_region, 
                                     id_var = "Subject_ID", 
                                     time_var = "timepoint", 
                                     values_var = "value", 
                                     group_var = "diagnosis", 
