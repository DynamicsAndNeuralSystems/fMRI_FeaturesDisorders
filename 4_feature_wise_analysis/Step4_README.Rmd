...
if (!file.exists(paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample.Rds"))) {
  feature_wise_SVM_in_sample <- run_in_sample_ksvm_by_feature(rdata_path = rdata_path,
  ...
  saveRDS(feature_wise_SVM_in_sample, file=paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample.Rds"))
} else {
  feature_wise_SVM_in_sample <- readRDS(paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample.Rds"))
}
```
...
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample_reweight.Rds"))) {
  feature_wise_SVM_in_sample_inv_prob <- run_in_sample_ksvm_by_feature(rdata_path = rdata_path,
                                                                         svm_kernel = svm_kernel,
                                                                         noise_procs = noise_procs,
    ...
    dplyr::rename("accuracy" = "Accuracy",
                  "balanced_accuracy" = "Balanced_Accuracy")
  saveRDS(feature_wise_SVM_in_sample_inv_prob, file=paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample_reweight.Rds"))
} else {
  feature_wise_SVM_in_sample_inv_prob <- readRDS(paste0(rdata_path, "UCLA_multivar_feature_res_svmLinear_in_sample_reweight.Rds"))
}
```
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Feature_Wise_SVM_Multi_ROI_Inv_Prob.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_upsampled.Rds"))) {
  set.seed(127)
  feature_wise_SVM_in_sample_upsampled <- run_in_sample_ksvm_by_feature(rdata_path = rdata_path,
                                                                         svm_kernel = svm_kernel,
                                                                         noise_procs = noise_procs,
                                                                      use_inv_prob_weighting = FALSE,
    ...
    dplyr::rename("accuracy" = "Accuracy",
                  "balanced_accuracy" = "Balanced_Accuracy")
  saveRDS(feature_wise_SVM_in_sample_upsampled, file=paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_upsampled.Rds"))
} else {
  feature_wise_SVM_in_sample_upsampled <- readRDS(paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_upsampled.Rds"))
}
```
...
# Control subject proportion is highlighted for accuracy
noise_procs = c("AROMA+2P", "AROMA+2P+GMR", "AROMA+2P+DiCER")
plot_class_acc_w_props(class_res = feature_wise_SVM_in_sample_upsampled,
                       cv = FALSE,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Feature_Wise_SVM_Multi_ROI_Upsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_downsampled.Rds"))) {
  set.seed(127)
  feature_wise_SVM_in_sample_downsampled <- run_in_sample_ksvm_by_feature(rdata_path = rdata_path,
                                                                        svm_kernel = svm_kernel,
                                                                        noise_procs = noise_procs,
                                                                        use_inv_prob_weighting = FALSE,
                                                                        upsample_minority = FALSE,
    ...
    dplyr::rename("accuracy" = "Accuracy",
                  "balanced_accuracy" = "Balanced_Accuracy")
  saveRDS(feature_wise_SVM_in_sample_downsampled, file=paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_downsampled.Rds"))
} else {
  feature_wise_SVM_in_sample_downsampled <- readRDS(paste0(rdata_path, "UCLA_multi_ROI_feature_res_svmLinear_in_sample_downsampled.Rds"))
}
```
...
# Control subject proportion is highlighted for accuracy
noise_procs = c("AROMA+2P", "AROMA+2P+GMR", "AROMA+2P+DiCER")
plot_class_acc_w_props(class_res = feature_wise_SVM_in_sample_downsampled,
                       cv = FALSE,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Feature_Wise_SVM_Multi_ROI_Downsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV.Rds"))) {
  
   feature_wise_SVM_caret <- run_caret_multi_SVM_by_feature(rdata_path = rdata_path,
  ...
  saveRDS(feature_wise_SVM_caret, file=paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV.Rds"))
} else {
  feature_wise_SVM_caret <- readRDS(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV.Rds"))
}
```
```{r, fig.width=7, fig.height=6}
# Plot accuracy + balanced accuracy in histograms
# Control subject proportion is highlighted for accuracy
noise_procs = c("AROMA+2P", "AROMA+2P+GMR", "AROMA+2P+DiCER")
plot_class_acc_w_props(class_res = feature_wise_SVM_caret,
                       cv = TRUE,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/Caret_10CV_Feature_Wise_SVM_Multi_Region.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_inv_prob.Rds"))) {
  
   feature_wise_SVM_caret_inv_prob_df <- run_caret_multi_SVM_by_feature(rdata_path = rdata_path,
                                            use_inv_prob_weighting = use_inv_prob_weighting,
                                            upsample_minority = upsample_minority,
                                            downsample_majority = downsample_majority,
                                            noise_procs = noise_procs)  %>%
  ...
  saveRDS(feature_wise_SVM_caret_inv_prob_df, file=paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_inv_prob.Rds"))
} else {
  feature_wise_SVM_caret_inv_prob_df <- readRDS(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_inv_prob.Rds"))
}
```
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Feature_Wise_SVM_Multi_Region_Inv_Prop.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_upsampled.Rds"))) {
  
   feature_wise_SVM_caret_upsampled <- run_caret_multi_SVM_by_feature(rdata_path = rdata_path,
                                            use_inv_prob_weighting = use_inv_prob_weighting,
                                            upsample_minority = upsample_minority,
                                            downsample_majority = downsample_majority,
                                            noise_procs = noise_procs)  %>%
  ...
  saveRDS(feature_wise_SVM_caret_upsampled, file=paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_upsampled.Rds"))
} else {
  feature_wise_SVM_caret_upsampled <- readRDS(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_upsampled.Rds"))
}
```

```{r, fig.width=7, fig.height=6}
plot_class_acc_w_props(class_res = feature_wise_SVM_caret_upsampled,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Feature_Wise_SVM_Multi_Region_Upsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
if (!file.exists(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_downsampled.Rds"))) {
  
   feature_wise_SVM_caret_downsampled <- run_caret_multi_SVM_by_feature(rdata_path = rdata_path,
                                            use_inv_prob_weighting = use_inv_prob_weighting,
                                            upsample_minority = upsample_minority,
                                            downsample_majority = downsample_majority,
                                            noise_procs = noise_procs)  %>%
  ...
  saveRDS(feature_wise_SVM_caret_downsampled, file=paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_downsampled.Rds"))
} else {
  feature_wise_SVM_caret_downsampled <- readRDS(paste0(rdata_path, "UCLA_feature_wise_multi_ROI_svmLinear_CV_downsampled.Rds"))
}
```

```{r, fig.width=7, fig.height=6}
plot_class_acc_w_props(class_res = feature_wise_SVM_caret_downsampled,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Feature_Wise_SVM_Multi_Region_Downsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
```
```{r, fig.width=7, fig.height=6}
feature_wise_SVM_caret_upsampled %>%
...
```{r}
if (!file.exists(paste0(rdata_path, "UCLA_feature_wise_SVM_caret_upsampled_pvals.Rds"))) {
  feature_wise_SVM_caret_upsampled_pvals <- calc_empirical_nulls(class_res = feature_wise_SVM_caret_upsampled,
                                                              null_data = model_free_shuffle_null_upsampled)
  saveRDS(feature_wise_SVM_caret_upsampled_pvals, file=paste0(rdata_path, "UCLA_feature_wise_SVM_caret_upsampled_pvals.Rds"))
  
} else {
  feature_wise_SVM_caret_upsampled_pvals <- readRDS(paste0(rdata_path, "UCLA_feature_wise_SVM_caret_upsampled_pvals.Rds"))
}

feature_wise_SVM_caret_upsampled_plabs <- feature_wise_SVM_caret_upsampled_pvals %>%
...
top_features <- feature_wise_SVM_caret_upsampled %>%
...
feature_wise_SVM_caret_upsampled_pvals %>%
  filter(Noise_Proc == "AROMA+2P+DiCER",
         catch22_Feature %in% top_features) %>%
  ggplot(data=.) +
  ...
  geom_text(data = subset(feature_wise_SVM_caret_upsampled_plabs, 
...
```
```{r, echo=F, eval=F}
ggsave("plots/Main_vs_Null_Acc_AROMA_2P_DiCER_Top5_catch22_Features.png", width=10, 
...
```
```{r, echo=F, eval=F}
ggsave("plots/Main_vs_Null_Bal_Acc_AROMA_2P_DiCER_Top5_catch22_Features.png", width=10, 
       height=7, units="in", dpi=300)
```
...
```{r}
accuracy_summary <- left_join(feature_wise_SVM_caret_upsampled, feature_wise_SVM_caret_upsampled_plabs) %>%
  dplyr::select(catch22_Feature, Noise_Proc, accuracy, acc_p, acc_p_adj) %>%
  filter(Noise_Proc == "AROMA+2P+DiCER") %>%
  mutate(accuracy = 100*round(accuracy, 4)) %>%
  arrange(desc(accuracy))

# write.csv(accuracy_summary, "Step4_Accuracy_Summary.csv", row.names = F)
```