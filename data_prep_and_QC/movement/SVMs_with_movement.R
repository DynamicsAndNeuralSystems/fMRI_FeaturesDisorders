...
UCLA_seq_list <- seq(0.12, max(subset(UCLA_ABIDE_movement_data,
                                      Study=="UCLA_CNP") %>%
                                 pull(Power)), by=0.02)
ABIDE_seq_list <- seq(0.01, 1, by=0.02)
...
UCLA_mvmt_svm_res <- run_repeat_cv_linear_svm(movement_data = UCLA_movement_data,
...
UCLA_ROI_catch22_svm_res <- run_repeat_cv_linear_svm(movement_data = UCLA_movement_data,
                                                     mvmt_list = UCLA_seq_list,
                                                     catch22_data = UCLA_catch22_zscored,
                                                     movement_var = "Power",
                                                     type = "Brain Region",
...
UCLA_combo_catch22_svm_res <- run_repeat_cv_linear_svm(
  mvmt_list = UCLA_seq_list,
  movement_data = UCLA_movement_data,
  catch22_data = UCLA_catch22_zscored,
  movement_var = "Power",
  type = "Combo",
  sample_groups = UCLA_sample_groups
...
ABIDE_mvmt_svm_res <- run_repeat_cv_linear_svm(
  ...
  movement_data = ABIDE_movement_data,
