...
def tenFoldCVScore(X,y):
    ...
    for trainIndices, testIndices in skf.split(X,y):
        ...
        try:
            ...
            print("ValueError. Showing classes in y.")
...
#-------------------------------------------------------------------------------
def tValueHistograms(dataset, featCount, featNames, roiCount, subjCount, c22Data, subjIndicesBelowThresh, labelColumn):
...
def accuracyOfRegions(roiCount, c22Data, subjIndicesBelowThresh, labelColumn, dispFigs, returnDF=False):
    ...
    which five regions were the most accurate. Can optionally return the sorted
    (by accuracy) data frame of all region accuracies/errors.'''
...
def featByFeatAnalysis(featureName, c22Data, labelColumn, roiCount, subjCount, subjIndicesBelowThresh):
    ...
    displayViolinPlots(labelColumn, bestPValInds, featSlice, 0, feature)
...
#-------------------------------------------------------------------------------
def kiloLabelShufflesAndLearns(labelColumn, c22Data, subjIndicesBelowThresh, roiCount, subjCount, featNames, featCount=22):
...
def featureAccuracyPVals(dfFeatAccsUCLA,dfFeatAccsCOBRE,randomLearnDataUCLA,randomLearnDataCOBRE,featCount):
    ...
    pValsCorrectedUCLA = multitest.multipletests(pValsUCLA, method='fdr_bh')[1]
    pValsCorrectedCOBRE = multitest.multipletests(pValsCOBRE, method='fdr_bh')[1]
    df = pd.DataFrame({'UCLA P Values':pValsCorrectedUCLA, 'COBRE P Values':pValsCorrectedCOBRE})
    df = df.round(3)
    ...
    # Multiple hypothesis test correction. Benjamini/Hochberg (non-negative)
    pValsCombined = multitest.multipletests(pValsCombined, method='fdr_bh')[1]
