...
if (!file.exists(paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob.Rds"))) {
  Feature_wise_unweighted_SVM_inv_prob_by_PC_list <- list()
  ...
  for (this_feature in unique(feature_matrix$names)) {
    ...
    # Append this feature's linear SVM results to list
    Feature_wise_unweighted_SVM_inv_prob_by_PC_list <- rlist::list.append(Feature_wise_unweighted_SVM_inv_prob_by_PC_list,
                                                                         df_res)
  }

  # Combine SVM results across features
  Feature_wise_PCA_linear_SVM_unweighted_inv_prob <- do.call(plyr::rbind.fill, Feature_wise_unweighted_SVM_inv_prob_by_PC_list)
  saveRDS(Feature_wise_PCA_linear_SVM_unweighted_inv_prob, paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob.Rds"))
} else {
  Feature_wise_PCA_linear_SVM_unweighted_inv_prob <- readRDS(paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob.Rds"))
}
```
...
Plot balanced accuracy across different values of C:
```{r}
Feature_wise_PCA_linear_SVM_unweighted_diffC %>%
  ...
  ggtitle("Linear SVM with Feature-wise PCA\nby SVM C Value") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = "bottom")
ggsave("plots/Feature_Wise_PCA_by_C_Value_Linear_SVM_Balanced_Accuracy_diffC.png", width=6, height=5, 
       units="in", dpi=300)
```
...
if (!file.exists(paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob_diffC.Rds"))) {
  ...
  # Combine SVM results across features
  Feature_wise_PCA_linear_SVM_unweighted_inv_prob_diffC <- do.call(plyr::rbind.fill, Feature_wise_unweighted_SVM_inv_prob_by_PC_list)
  saveRDS(Feature_wise_PCA_linear_SVM_unweighted_inv_prob_diffC, paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob_diffC.Rds"))
} else {
  Feature_wise_PCA_linear_SVM_unweighted_inv_prob_diffC <- readRDS(paste0(rdata_path, "Feature_wise_PCA_linear_SVM_inv_prob_diffC.Rds"))
}
```
...
Feature_wise_PCA_linear_SVM_unweighted_inv_prob_diffC %>%
  ...
  xlab("# PCs Used in Linear SVM") +
  facet_wrap(c_value ~ ., scales="free") +
  ggtitle("Inv Prob Linear SVM with Feature-wise PCA\nby SVM C Value") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = "bottom")
ggsave("plots/Feature_Wise_PCA_In_Sample_Inv_Prob_Linear_SVM_Balanced_Accuracy_diffC.png", width=6, height=5, 
       units="in", dpi=300)
```
