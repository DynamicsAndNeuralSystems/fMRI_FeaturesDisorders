...
if (!file.exists(paste0(rdata_path, "Model_Permutation_Null_In_Sample.Rds"))) {
  ...
  saveRDS(model_permutation_null_in_sample, file=paste0(rdata_path, "Model_Permutation_Null_In_Sample.Rds"))
} else {
  model_permutation_null_in_sample <- paste0(rdata_path, "Model_Permutation_Null_In_Sample.Rds")
}
```

Plot null accuracy and balanced accuracy compared with real metrics:
```{r, fig.width=7, fig.height=6}
e1071_region_wise_SVM_in_sample %>%
  dplyr::select(grouping_var, Noise_Proc, accuracy, balanced_accuracy) %>%
  mutate(Type = "main") %>%
  ...
  geom_histogram(aes(fill = Type, y=0.5*..density..), 
                 ...
                 alpha=0.6, position="identity") +
  facet_grid(Noise_Proc ~ Metric, switch="y", scales="free_x") +
  xlab("Value from n=8,200 Pooled Null Iterations") +
  ylab("Scaled Density") +
  labs(fill = "Distribution") +
  theme(strip.text.y.left = element_text(angle=0),
        strip.placement = "outside",
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Main_vs_Pooled_Permutation_Null_Acc_BAcc_In_Sample_ROI_Wise_Linear_SVM.png",
       width=7, height=6, units="in", dpi=300)
```

#### In-sample with inverse probability weighting
...
if (!file.exists(paste0(rdata_path, "Model_Permutation_Null_In_Sample_Inv_Prob.Rds"))) {
  ...
  saveRDS(model_permutation_null_in_sample_inv_prob, file=paste0(rdata_path, "Model_Permutation_Null_In_Sample_Inv_Prob.Rds"))
} else {
  model_permutation_null_in_sample_inv_prob <- paste0(rdata_path, "Model_Permutation_Null_In_Sample_Inv_Prob.Rds")
}
```

Plot null accuracy and balanced accuracy compared with real metrics:
```{r, fig.width=7, fig.height=6}
e1071_region_wise_SVM_in_sample_inv_prob %>%
  dplyr::select(grouping_var, Noise_Proc, accuracy, balanced_accuracy) %>%
  mutate(Type = "main") %>%
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Main_vs_Pooled_Permutation_Null_Acc_BAcc_In_Sample_ROI_Wise_Linear_SVM_Inv_Prob.png",
       width=7, height=6, units="in", dpi=300)
```
...
if (!file.exists(paste0(rdata_path, "Model_Permutation_Null_CV.Rds"))) {
  model_permutation_null_CV <- run_null_model_n_permutations_CV(rdata_path,
                                                                noise_procs = noise_procs,
                                                                grouping_var = "Brain_Region",
                                                                svm_feature_var = "Feature",
                                                                num_permutations = 100,
  ...
  saveRDS(model_permutation_null_CV, file=paste0(rdata_path, "Model_Permutation_Null_CV.Rds"))
} else {
  model_permutation_null_CV <- paste0(rdata_path, "Model_Permutation_Null_CV.Rds")
}
```

#### Cross-validated with inverse probability weighting
...
real_82_acc <- e1071_region_wise_SVM_CV %>%
  filter(Noise_Proc=="AROMA+2P") %>%
  summarise(mean_accuracy = mean(accuracy, na.rm=T),
            mean_balanced_accuracy = mean(balanced_accuracy, na.rm=T)) %>%
  mutate(Type="main")
...
real_82_acc <- e1071_region_wise_SVM_CV_inv_prob %>%
  filter(Noise_Proc=="AROMA+2P") %>%
  summarise(mean_accuracy = mean(accuracy, na.rm=T),
            mean_balanced_accuracy = mean(balanced_accuracy, na.rm=T)) %>%
  mutate(Type="main")
