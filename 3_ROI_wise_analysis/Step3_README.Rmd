...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/In_Sample_Region_Wise_linear_SVM_Multi_Feature.png",
...
if (!file.exists(paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_in_sample_reweight.Rds"))) {
  region_wise_SVM_in_sample_inv_prob <- run_in_sample_ksvm_by_region(rdata_path = rdata_path,
                                                                         svm_kernel = svm_kernel,
                                                                         noise_procs = noise_procs,
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Region_Wise_SVM_Multi_Feature_Inv_Prob.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Region_Wise_SVM_Multi_Feature_Upsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/In_Sample_Region_Wise_SVM_Multi_Feature_Downsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, echo=F, eval=F}
# Save plot
ggsave("plots/Caret_10CV_Region_Wise_SVM_Multi_Feature.png",
       width=7, height=6, units="in", dpi=300)
```
...
# Run theft's multivariable classifier on each ROI and save to an RDS object
# If the RDS object doesn't already exist, otherwise load it in
for (method in c("e1071", "kernlab")) {
  if (!file.exists(paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_inv_prob_", method, ".Rds"))) {
    ...
    saveRDS(region_wise_SVM_caret_inv_prob_df, file=paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_inv_prob_", method, ".Rds"))
  } else {
    region_wise_SVM_caret_inv_prob_df <- readRDS(paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_inv_prob_", method, ".Rds"))
  }
}
...
```{r}
# e1071
region_wise_SVM_caret_inv_prob_1071 <- readRDS(paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_inv_prob_e1071.Rds"))
plot_class_acc_w_props(class_res = region_wise_SVM_caret_inv_prob_1071,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
```{r}
# kernlab
region_wise_SVM_caret_inv_prob_kernlab <- readRDS(paste0(rdata_path, "UCLA_multivar_ROI_res_svmLinear_inv_prob_kernlab.Rds"))
plot_class_acc_w_props(class_res = region_wise_SVM_caret_inv_prob_kernlab,
                       rdata_path = rdata_path,
                       noise_procs = noise_procs)
```
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Region_Wise_SVM_Multi_Feature_Reweighting.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Region_Wise_SVM_Multi_Feature_Upsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Caret_10CV_Region_Wise_SVM_Multi_Feature_Downsampled.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r, eval=F, echo=F}
# Save plot
ggsave("plots/Main_vs_Null_Acc_BAcc_Caret_10FCV_Linear_SVM_ROI_Wise.png",
       width=7, height=6, units="in", dpi=300)
```
...
```
```{r, echo=F, eval=F}
ggsave("plots/Main_vs_Null_Acc_AROMA_2P_DiCER_Top5.png", width=10, 
...
We can run in-sample linear SVM with minority upsampling for each feature individually.
...
We can also run the linear SVM with 10-fold cross-validation and inverse probability weighting for each feature individually.
...
```{r, echo=F, eval=F}
ggsave("plots/Caret_10CV_Region_Wise_SVM_Uni_Feature_Reweighting.png",
       width=7, height=6, units="in", dpi=300)
```

These panels show the results from in-sample (upper) and 10-fold CV (lower) linear SVM using each catch22 feature separately (i.e. univariable SVMs) -- both of which are using minority class upsampling. Of note while there are peaks around 0.5 for accuracy and 0.25 for balanced accuracy, there are a good number of feature X ROI combinations with accuracies and balanced accuracies greater than change.


### Comparing individual features with resampled null distribution

We can compare the in-sample accuracies and balanced accuracies for single-feature SVM with the minority upsampled null distribution:
...
```
```{r, echo=F, eval=F}
ggsave("plots/Main_vs_Null_Acc_BAcc_Caret_In_Sample_Linear_SVM_Uni_Feature_ROI_Wise.png",
       width=7, height=6, units="in", dpi=300)
```
...
```
```{r, echo=F, eval=F}
ggsave("plots/Main_vs_Null_Acc_BAcc_Caret_10FCV_Linear_SVM_Uni_Feature_ROI_Wise.png",
       width=7, height=6, units="in", dpi=300)
```
...
```{r}
accuracy_summary <- left_join(region_wise_SVM_caret_upsampled, region_wise_SVM_caret_upsampled_plabs) %>%
  dplyr::select(Brain_Region, Noise_Proc, accuracy, acc_p, acc_p_adj) %>%
  filter(Noise_Proc == "AROMA+2P+DiCER") %>%
  mutate(accuracy = 100*round(accuracy, 4)) %>%
  arrange(desc(accuracy))

write.csv(accuracy_summary, "Step3_Accuracy_Summary.csv", row.names = F)
```