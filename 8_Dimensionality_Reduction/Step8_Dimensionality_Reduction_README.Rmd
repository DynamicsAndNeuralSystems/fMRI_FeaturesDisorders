...
catch22_feature_matrix <- readRDS(paste0(rdata_path, 
                                         sprintf("UCLA_%s_catch22_zscored_filtered.Rds", 
                                                             noise_label)))     
...
if (!file.exists(paste0(rdata_path, "Combo_wise_PCA_linear_SVM_unweighted_catch22.Rds"))) {
  ...
  saveRDS(Combo_wise_PCA_linear_SVM_unweighted, paste0(rdata_path, "Combo_wise_PCA_linear_SVM_unweighted_catch22.Rds"))
} else {
  Combo_wise_PCA_linear_SVM_unweighted <- readRDS(paste0(rdata_path, "Combo_wise_PCA_linear_SVM_unweighted_catch22.Rds"))
}
```
...
if (!file.exists(paste0(rdata_path, "Combo_wise_PCA_linear_SVM_inv_prob_catch22.Rds"))) {
  ...
  saveRDS(Combo_wise_PCA_linear_SVM_inv_prob, paste0(rdata_path, "Combo_wise_PCA_linear_SVM_inv_prob_catch22.Rds"))
} else {
  Combo_wise_PCA_linear_SVM_inv_prob <- readRDS(paste0(rdata_path, "Combo_wise_PCA_linear_SVM_inv_prob_catch22.Rds"))
}
```
...
loess_X <- Combo_wise_PCA_linear_SVM_inv_prob %>% 
  filter(Sample_Type=="In-sample") %>%
  pull(Num_PCs)
loess_Y <- in_sample_balanced_accuracy_loess$fitted
...
p_loess <- ggplot(data = data.frame(x = loess_X,
  ...
  xlab("Number of PCs") +
  geom_line() +
  geom_vline(xintercept = 69, linetype = 2, color = "red") +
  theme(plot.title = element_text(hjust=0.5))
...
Combo_wise_PCA_res_combined %>%
  filter(Num_PCs %in% c(total_num_pcs, pcs_with_balanced_accuracy_1),
         Sample_Type == "Out-of-sample") %>%
  dplyr::select(Method, Num_PCs, balanced_accuracy, balanced_accuracy_SD) %>%
  mutate(PC_stop_point = ifelse(Num_PCs == pcs_with_balanced_accuracy_1,
...
full_data_for_SVM <- feature_matrix %>%
  filter(Brain_Region == example_ROI) %>%
  pivot_wider(id_cols = c(Subject_ID, group),
              names_from = names,
              values_from = values) %>%
  dplyr::select(-Subject_ID) %>%
  ...
  as.matrix() 
first_20_PCs_for_SVM <- ROI_PC_scores %>%
  filter(Brain_Region == example_ROI) %>%
  dplyr::select(group, PC1:PC20)
