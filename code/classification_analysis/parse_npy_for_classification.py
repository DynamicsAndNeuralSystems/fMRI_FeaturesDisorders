...
parser.add_argument('--analysis_type', dest='analysis_type', default="Brain_Region")
...
parser.add_argument('--null_starting_point', dest='null_starting_point')
parser.add_argument('--random_num_for_perm', dest='random_num_for_perm', default=127)
...
null_starting_point = int(args.null_starting_point)
random_num_for_perm = int(args.random_num_for_perm)
...
main_output_file_base = f"{feature_name}_{classifier_type}_{num_repeats}_repeats_{num_folds}_folds_CV"
...
# Main analysis
if not os.path.isfile(f"{output_data_path}/null_results/{main_output_file_base}_nulls_from_{null_starting_point}.feather"):
    print(f"Now running {classifier_type} for {feature_name} null models")
    main_classification_res, splits_df, null_classification_res = run_k_fold_classifier_for_feature(feature_data = feature_data, 
                                                                                        ...
                                                                                        grouping_var_name=feature_name,
                                                                                        scorers=scorers,
                                                                                        scoring_names=scoring_names,
                                                                                        num_null_iters=num_null_iters,
                                                                                        num_folds = 10,
                                                                                        num_repeats = 10,
                                                                                        random_num_for_perm = random_num_for_perm,
                                                                                        num_jobs = num_jobs)
    
    # Assign key details to dataframes
    main_classification_res["Analysis_Type"] = analysis_type
    main_classification_res["Feature_Name"] = feature_name
    main_classification_res["Classifier_Type"] = classifier_type

    null_classification_res["Analysis_Type"] = analysis_type
    null_classification_res["Feature_Name"] = feature_name
    null_classification_res["Classifier_Type"] = classifier_type
    
    # Save results to feather file
    if not os.path.isfile(f"{output_data_path}/balanced_accuracy/{main_output_file_base}_main_res.feather"):
        main_classification_res.to_feather(f"{output_data_path}/balanced_accuracy/{main_output_file_base}_main_res.feather")
    null_classification_res.to_feather(f"{output_data_path}/null_results/{main_output_file_base}_nulls_from_{null_starting_point}.feather")
