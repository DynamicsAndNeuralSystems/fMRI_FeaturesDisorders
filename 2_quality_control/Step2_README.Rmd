...
One thing to look at is how many subjects had `NA` values per feature:

```{r}
NA_feature_data_catchaMouse16 <- find_univariate_feature_na(rdata_path = rdata_path,
                                                            feature_set = "catchaMouse16")

NA_feature_data_catchaMouse16
```

The feature `ST_LocalExtrema_n100_diffmaxabsmin` is NA across all 171 subjects.
...
NA_subjects_data <- find_univariate_feature_na(rdata_path = rdata_path,
                                               feature_set = "catch22")
NA_subjects_data
```
