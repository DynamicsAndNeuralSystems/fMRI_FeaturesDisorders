...
parser$add_argument("--uni_feature_set", default="catch22")
...
uni_feature_set <- args$uni_feature_set
...
for (i in 1:nrow(weighting_param_df)) {
  ...
  if (!file.exists(paste0(rdata_path, sprintf("pyspi_SPI_pairwise_CV_linear_SVM_%s_%s.Rds",
    ...
    pyspi_SPI_pairwise_SVM_CV_weighting <- run_combined_uni_pairwise_cv_svm_by_input_var(univariate_data = univariate_data,
