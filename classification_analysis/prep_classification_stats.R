...
compare_main_and_null <- function(main_df_iter, null_distribution_df) {
  ...
  null_distribution_df <- null_distribution_df %>%
    dplyr::select(-index, -group_var, -kernel, -Null_Iter_Number) %>%
...
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_{scaler}_scaler_balanced_accuracy_AUC_all_folds.feather"))) {
  ...
  for (i in 1:nrow(study_group_df)) {
    ...
    for (kernel in SVM_kernels) {
      balacc_AUC_across_folds <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/processed_data/{dataset_ID}_{comparison_group}_Univariate_{univariate_feature_set}_{scaler}_scaler_{kernel}_SVM_balanced_accuracy_AUC.feather"))
      balacc_AUC_across_folds$Study <- dataset_ID
      balacc_AUC_across_folds$kernel <- kernel
...
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_{scaler}_scaler_fold_assignments.feather"))) {
  ...
  for (i in 1:nrow(study_group_df)) {
    ...
    # Now iterate over each univariate feature set
    fold_assignments <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/processed_data/{dataset_ID}_{comparison_group}_Univariate_{univariate_feature_set}_{scaler}_scaler_linear_SVM_fold_assignments.feather"))
...
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_{scaler}_scaler_ROC_TPR_FPR.feather"))) {
  ...
  # Now iterate over each univariate feature set
    ROC_data <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/processed_data/{dataset_ID}_{comparison_group}_Univariate_{univariate_feature_set}_{scaler}_scaler_linear_SVM_ROC_TPR_FPR.feather"))
...
# Univariate
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_{scaler}_scaler_linear_SVM_coefficients.feather"))) {
...
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_{scaler}_scaler_null_balanced_accuracy_distributions.feather"))) {
  ...
  for (i in 1:nrow(study_group_df)) {
    ...
    null_dist <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/processed_data/{dataset_ID}_{comparison_group}_Univariate_{univariate_feature_set}_{scaler}_scaler_linear_SVM_null_balanced_accuracy_distributions.feather"))
