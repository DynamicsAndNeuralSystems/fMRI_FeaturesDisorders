...
compare_main_and_null <- function(main_df_iter, null_distribution_df) {
  ...
  null_distribution_df <- null_distribution_df %>%
      dplyr::select(Analysis_Type, Comparison_Group, Study, Null_Balanced_Accuracy, group_var) %>%
      semi_join(., main_df_iter %>% dplyr::select(Analysis_Type, Comparison_Group, Study, Balanced_Accuracy_Across_Folds, group_var))
  ...
  p_value <- pnorm(q=main_balanced_accuracy_value, mean=null_mean, sd=null_SD, lower.tail = FALSE)
...
################################################################################
# Compile proportion of correct predictions
################################################################################

# Univariate
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_prop_correct_preds.feather"))) {
  univariate_prop_correct_preds_list <- list()
  # First iterate over each study/comparison group
  for (i in 1:nrow(study_group_df)) {
    dataset_ID <- study_group_df$Study[i]
    noise_proc <- study_group_df$Noise_Proc[i]
    ...
    # Now iterate over each univariate feature set
    prop_correct_preds <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/classification_results/sample_predictions/{dataset_ID}_{comparison_group}_Univariate_{univariate_feature_set}_Linear_SVM_prop_correct_pred.feather"))
    prop_correct_preds$Study <- dataset_ID
    prop_correct_preds$Univariate_Feature_Set <- univariate_feature_set
    # Append to list
    univariate_prop_correct_preds_list <- list.append(univariate_prop_correct_preds_list, prop_correct_preds)

  }

  # Combine the list results into a dataframe
  univariate_prop_correct_preds <- do.call(plyr::rbind.fill, 
                                                    univariate_prop_correct_preds_list) %>%
    mutate(Analysis_Type = case_when(str_detect(group_var, "_") ~ "TS_Feature",
                                     group_var == "Combo" ~ "Combo",
                                     T ~ "Brain_Region"))
  
  # Save to feather file
  feather::write_feather(univariate_prop_correct_preds,
                         glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_prop_correct_preds.feather"))
} else {
  univariate_prop_correct_preds <- feather::read_feather(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_univariate_prop_correct_preds.feather"))
}

# Pairwise
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_pairwise_prop_correct_preds.feather"))) {
  pairwise_prop_correct_preds_list <- list()
  # First iterate over each study/comparison group
  for (i in 1:nrow(study_group_df)) {
    dataset_ID <- study_group_df$Study[i]
    noise_proc <- study_group_df$Noise_Proc[i]
    ...
    prop_correct_preds <- pyarrow_feather$read_feather(glue("{data_path}/{dataset_ID}/classification_results/sample_predictions/{dataset_ID}_{comparison_group}_Pairwise_{pairwise_feature_set}_Linear_SVM_prop_correct_pred.feather"))
    prop_correct_preds$Study <- dataset_ID
    prop_correct_preds$Pairwise_Feature_Set <- pairwise_feature_set
    # Append to list
    pairwise_prop_correct_preds_list <- list.append(pairwise_prop_correct_preds_list, prop_correct_preds)
  }
  
  # Combine the list results into a dataframe
  pairwise_prop_correct_preds <- do.call(plyr::rbind.fill, 
                                                    pairwise_prop_correct_preds_list) %>%
    mutate(Analysis_Type = "Pairwise_SPI")
  
  # Save to feather file
  feather::write_feather(pairwise_prop_correct_preds,
                         glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_pairwise_prop_correct_preds.feather"))
} else {
  pairwise_prop_correct_preds <- feather::read_feather(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_pairwise_prop_correct_preds.feather"))
}

# Univariate + Pairwise combined
if (!file.exists(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_combined_univariate_pairwise_prop_correct_preds.feather"))) {
  combined_univariate_pairwise_prop_correct_preds_list <- list()
  # First iterate over each study/comparison group
  for (i in 1:nrow(study_group_df)) {
    dataset_ID <- study_group_df$Study[i]
    noise_proc <- study_group_df$Noise_Proc[i]
    ...
    prop_correct_preds$Pairwise_Feature_Set <- pairwise_feature_set
    # Append to list
    combined_univariate_pairwise_prop_correct_preds_list <- list.append(combined_univariate_pairwise_prop_correct_preds_list, prop_correct_preds)
  }
  
  # Combine the list results into a dataframe
  combined_univariate_pairwise_prop_correct_preds <- do.call(plyr::rbind.fill, 
                                                    combined_univariate_pairwise_prop_correct_preds_list) %>%
    mutate(Analysis_Type = "SPI_Univariate_Combo")
  
  # Save to feather file
  feather::write_feather(combined_univariate_pairwise_prop_correct_preds,
                         glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_combined_univariate_pairwise_prop_correct_preds.feather"))
} else {
  combined_univariate_pairwise_prop_correct_preds <- feather::read_feather(glue("{output_data_path}/UCLA_CNP_ABIDE_ASD_combined_univariate_pairwise_prop_correct_preds.feather"))
}
